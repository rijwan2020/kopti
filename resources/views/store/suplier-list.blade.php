...
<div class="ui-bordered px-3 pt-3 mb-3">
    <form class="form-row align-items-center" method="get" action="{{ url()->current() }}">
        ...
        <div class="col-md text-right">
            @if (Auth::user()->hasRule('suplierAdd'))
                <a href="{{ route('suplierAdd') }}" class="btn btn-info" data-toggle="tooltip" data-state="dark" title="Tambah data suplier">
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header h4 text-center">Data Suplier</div>
                        ...
                        @foreach ($data['data'] as $value)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if (Auth::user()->hasRule('suplierEdit'))
                                        <a href="{{ route('suplierEdit', ['id' => $value->id]) }}" class="btn icon-btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" data-state="dark" title="Edit suplier">
                                            <i class="fa fa-pen"></i>
                                        </a>
                                    @endif	
                                    @if (Auth::user()->hasRule('suplierDelete'))
                                        <a href="#" class="btn btn-sm icon-btn btn-danger data-delete" data-state="dark" data-toggle="tooltip" data-placement="top" data-url="{{ route('suplierDelete', ['id'=>$value->id]) }}" title="Hapus suplier" data-message="Anda yakin akan menghapus data suplier {{ $value->name }}?">
