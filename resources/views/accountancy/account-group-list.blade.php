...
<div class="ui-bordered px-3 pt-3 mb-3">
    <form class="form-row align-items-center" method="get" action="{{ url()->current() }}">
        ...
        <div class="col-md text-right">
            @if (Auth::user()->hasRule('accountGroupAdd'))
                <a href="{{ route('accountGroupAdd') }}" class="btn my-1 btn-primary" data-toggle="tooltip" data-state="dark" title="Tambah kelompok akun">
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header h4 text-center">Data Kelompok Akun</div>
                        ...
                        @foreach ($data['data'] as $value)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if (Auth::user()->hasRule('accountGroupEdit'))
                                        <a href="{{ route('accountGroupEdit', ['id' => $value->id]) }}" class="btn icon-btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" data-state="dark" title="Edit kelompok akun">
                                            <i class="fa fa-pen"></i>
                                        </a>
                                    @endif
                                    @if (Auth::user()->hasRule('accountGroupDelete') && $value->default==0)
                                        <a href="#" class="btn btn-sm icon-btn btn-danger data-delete" data-state="dark" data-toggle="tooltip" data-placement="top" data-url="{{ route('accountGroupDelete', ['id'=>$value->id]) }}" title="Hapus kelompok akun" data-message="Anda yakin akan menghapus kelompok akun : {{$value->name}}?">
