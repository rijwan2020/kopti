...
<div class="ui-bordered px-3 pt-3 mb-3">
    <form class="form-row align-items-center" method="get" action="{{ url()->current() }}">
        ...
        <div class="col-md text-right">
            ...
            <div class="row">
                <div class="col-md-12">
                    @if (Auth::user()->hasRule('adjustingJournalAdd') && !isset($data['tbb_id']) && !isset($data['tbt_id']))
                        <a href="{{ route('adjustingJournalAdd') }}" class="btn btn-info mb-3" data-toggle="tooltip" data-state="dark" data-placement="bottom" data-original-title="Tambah Jurnal Penyesuaian">
                            <i class="fa fa-plus"></i>
                            Tambah
                        </a>   
                    @endif
                    @if (Auth::user()->hasRule('adjustingJournalPrint'))
                        <a href="{{ route('adjustingJournalPrint', $data['param']) }}" class="btn btn-dark mb-3" data-toggle="tooltip" data-state="dark" data-placement="bottom" data-original-title="Print Jurnal Penyesuaian" target="_blank">
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header text-center">
                <h4 class="mb-1">Jurnal Penyesuaian</h4>
                ...
                <table class="table card-table table-bordered">
                    ...
                    <tbody>
                        ...
                        @foreach ($data['data'] as $value)
                            ...
                            @endphp
                            <tr style="{{$value->edited==1?'background-color: #ecffe6':($value->deleted == 1 ? 'background-color: #ffcccc':'')}}">
                                <td class="text-center">{{ $i }}</td>
                                <td>{{ $value->reference_number }}</td>
                                <td>
                                    {{ date('d M Y H:i:s' ,strtotime($value->transaction_date)) }} <br><br>
                                    Diinput oleh: <b>{!!$value->userInput->name ?? '<i>-<i>'!!}</b>
                                    @if ($value->edited == 1)
                                        <br>Diupdate Oleh: <b>{!!$value->userEdit->name ?? '<i>-<i>'!!}</b>
                                    @endif
                                </td>
                                <td>
                                ...
                                @if (!isset($data['tbb_id']) && !isset($data['tbt_id']))
                                    <td class="text-center">
                                        @if (Auth::user()->hasRule('adjustingJournalEdit') && $value->unit == 0)
                                            <a href="{{ route('adjustingJournalEdit', ['id' => $value->id]) }}" class="btn icon-btn btn-info btn-sm" data-state="dark" data-toggle="tooltip" data-placement="bottom" title="Edit jurnal penyesuaian {{$value->reference_number}}">
                                                <i class="fa fa-pen"></i>
                                            </a>
                                        @endif
                                        @if (Auth::user()->hasRule('adjustingJournalDelete') && $value->unit == 0)
                                            <a href="#" class="btn btn-sm icon-btn btn-danger data-delete" data-state="dark" data-toggle="tooltip" data-placement="top" data-url="{{ route('adjustingJournalDelete', ['id'=>$value->id]) }}" title="Hapus jurnal penyesuaian" data-message="Anda yakin akan menghapus jurnal penyesuaian : {{$value->reference_number}}?">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Jurnal Penyesuaian</h5>
