...
@section('content')
<div class="card">
    <div class="card-header h4 text-center">Form Jurnal Transaksi</div>
    <form action="{{ route('journalSave') }}" enctype="multipart/form-data" class="form-input" method="post" autocomplete="false" id="input-jurnal-form">
        <div class="card-body">
            @csrf
            <input type="hidden" name="mode" value="{{$data['mode']}}">
            @if ($data['mode']=='edit')
                <input type="hidden" name="id" value="{{$data['data']->id}}">   
                <input type="hidden" name="adjusting_journal_id" value="{{$data['data']->adjusting_journal_id}}">
            @endif
            <div class="row">
