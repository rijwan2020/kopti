...
<div class="ui-bordered px-3 pt-3 mb-3">
    <form class="form-row align-items-center" method="get" action="{{ url()->current() }}">
        ...
        <div class="col-md text-right">
            @if (Auth::user()->hasRule('managementPositionList'))
                <a href="{{ route('managementPositionList') }}" class="btn my-1 btn-primary" data-toggle="tooltip" data-state="dark" title="Manage Jabatan">
                    <i class="fa fa-cog"></i>
                    Manage Jabatan
                </a>
            @endif
            @if (Auth::user()->hasRule('managementAdd'))
                <a href="{{ route('managementAdd') }}" class="btn my-1 btn-primary" data-toggle="tooltip" data-state="dark" title="Tambah data pengurus">
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header h4 text-center">Data Pengurus</div>
                        ...
                        @foreach ($data['data'] as $value)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if (Auth::user()->hasRule('managementEdit'))
                                        <a href="{{ route('managementEdit', ['id' => $value->id]) }}" class="btn icon-btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" data-state="dark" title="Edit data pengurus">
                                            <i class="fa fa-pen"></i>
                                        </a>
                                    @endif	
                                    @if (Auth::user()->hasRule('managementDelete'))
                                        <a href="#" class="btn btn-sm icon-btn btn-danger data-delete" data-state="dark" data-toggle="tooltip" data-placement="top" data-url="{{ route('managementDelete', ['id'=>$value->id]) }}" title="Hapus data anggota" data-message="Anda yakin akan menghapus data pengurus : {{$value->member->name}}?">
