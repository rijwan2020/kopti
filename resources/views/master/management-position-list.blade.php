...
<div class="ui-bordered px-3 pt-3 mb-3">
    <form class="form-row align-items-center" method="get" action="{{ url()->current() }}">
        ...
        <div class="col-md text-right">
            @if (Auth::user()->hasRule('managementPositionAdd'))
                <a href="{{ route('managementPositionAdd') }}" class="btn my-1 btn-primary" data-toggle="tooltip" data-state="dark" title="Tambah jabatan">
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header h4 text-center">Manage Jabatan</div>
                        ...
                        @foreach ($data['data'] as $value)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if (Auth::user()->hasRule('managementPositionEdit'))
                                        <a href="{{ route('managementPositionEdit', ['id' => $value->id]) }}" class="btn icon-btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" data-state="dark" title="Edit data jabatan">
                                            <i class="fa fa-pen"></i>
                                        </a>
                                    @endif	
                                    @if (Auth::user()->hasRule('managementPositionDelete') && $value->default==0)
                                        <a href="#" class="btn btn-sm icon-btn btn-danger data-delete" data-state="dark" data-toggle="tooltip" data-placement="top" data-url="{{ route('managementPositionDelete', ['id'=>$value->id]) }}" title="Hapus data jabatan" data-message="Anda yakin akan menghapus data jabatan : {{$value->name}}?">
