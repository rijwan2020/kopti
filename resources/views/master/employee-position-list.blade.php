...
<div class="ui-bordered px-3 pt-3 mb-3">
    <form class="form-row align-items-center" method="get" action="{{ url()->current() }}">
        ...
        <div class="col-md text-right">
            @if (Auth::user()->hasRule('employeePositionAdd'))
                <a href="{{ route('employeePositionAdd') }}" class="btn my-1 btn-primary" data-toggle="tooltip" data-state="dark" title="Tambah Posisi">
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header h4 text-center">Manage Posisi</div>
                        ...
                        @foreach ($data['data'] as $value)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if (Auth::user()->hasRule('employeePositionEdit'))
                                        <a href="{{ route('employeePositionEdit', ['id' => $value->id]) }}" class="btn icon-btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" data-state="dark" title="Edit data Posisi">
                                            <i class="fa fa-pen"></i>
                                        </a>
                                    @endif	
                                    @if (Auth::user()->hasRule('employeePositionDelete') && $value->default==0)
                                        <a href="#" class="btn btn-sm icon-btn btn-danger data-delete" data-state="dark" data-toggle="tooltip" data-placement="top" data-url="{{ route('employeePositionDelete', ['id'=>$value->id]) }}" title="Hapus data Posisi" data-message="Anda yakin akan menghapus data Posisi : {{$value->name}}?">
